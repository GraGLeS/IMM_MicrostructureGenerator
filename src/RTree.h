...
#define ASSERT(cond)
#ifndef RTreeMin
  #define RTreeMin std::min
#endif //RTreeMin
#ifndef RTreeMax
  #define RTreeMax std::max
#endif //RTreeMax
...
typename RTREE_QUAL::Rect RTREE_QUAL::CombineRect(Rect* a_rectA, Rect* a_rectB)
{
  ...
  for(int index = 0; index < NUMDIMS; ++index)
  {
    newRect.m_min[index] = RTreeMin(a_rectA->m_min[index], a_rectB->m_min[index]);
    newRect.m_max[index] = RTreeMax(a_rectA->m_max[index], a_rectB->m_max[index]);
  }
